...
(defvar *ticker-state* :stopped)
(defvar *last-abs-time* (get-internal-real-time))
(defvar *next-abs-time* (get-internal-real-time))
(defvar *songpos* 0)
...
(defun measure-tick-time ()
  (assert (>= *next-abs-time* *last-abs-time*))
  (let ((intvl (/ (- *next-abs-time* *last-abs-time*) 1000 )))
    (setf *last-abs-time* *next-abs-time*)
    (setf *next-abs-time* (get-internal-real-time))
